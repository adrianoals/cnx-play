...
import { ArrowLeft, Check, Shield, Lock, Crown, Tag } from "lucide-react"
  ...
  const handlePayment = () => {
    ...
    setTimeout(() => {
      ...
      toast({
        ...
        description: "Complete o pagamento na nova aba para liberar seu acesso.",
        duration: 5000,
      })

      setTimeout(() => {
        localStorage.removeItem("has_seen_onboarding")
        localStorage.removeItem("last_seen_certificate_level")
    ...
    <div className="min-h-screen bg-background flex flex-col relative overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-6 py-6">
        <Button
          ...
          className="text-muted-foreground hover:text-foreground hover:bg-secondary/50 pl-0 gap-2"
        >
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center p-6 pb-20">
        <div className="text-center max-w-2xl mx-auto mb-12">
          ...
          <p className="text-muted-foreground text-lg">
            Invista em networking de qualidade e transforme suas conexões.
          </p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 max-w-6xl w-full mb-12 items-stretch">
          <PlanCard
            type="monthly"
            title="Plano Mensal"
            price="59,99"
        ...
        <div className="max-w-md w-full space-y-6">
          <div className="bg-card/80 rounded-2xl p-6 border border-border">
            <div className="flex justify-between items-center mb-4 pb-4 border-b border-border">
              <span className="text-muted-foreground">Plano Selecionado</span>
              <span className="text-foreground font-medium">
                {selectedPlan === "monthly" ? "Mensal" : selectedPlan === "annual" ? "Anual Premium" : "Start 2026 (Promo)"}
              </span>
            </div>
            <div className="flex justify-between items-center">
              ...
              <span className={`text-2xl font-bold ${selectedPlan === "visionary" ? "text-amber-400" : "text-blue-400"}`}>
                {selectedPlan === "monthly" ? "R$ 59,99" : selectedPlan === "annual" ? "R$ 497,00" : "R$ 365,00"}
              </span>
            </div>
          </div>
