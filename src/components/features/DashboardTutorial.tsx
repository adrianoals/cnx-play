...
const steps = [
  ...
  { title: "Reunião Diária", description: "Todo dia selecionamos um parceiro ideal para você. Confirme a reunião no card para ganhar pontos.", icon: Handshake, color: "bg-green-500" },
  ...
  return (
    <AnimatePresence>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
        <motion.div key={step} initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative w-full max-w-md bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden">
          <div className={`h-32 ${current.color} relative flex items-center justify-center`}>
            ...
            <button onClick={onClose} className="absolute top-4 right-4 text-white/70 hover:text-white p-1 rounded-full"><X className="h-5 w-5" /></button>
          </div>
          <div className="p-6 md:p-8">
            <div className="flex items-center justify-between mb-4">
              <span className="text-xs font-bold text-slate-500 uppercase">Passo {step + 1} de {steps.length}</span>
