...
export default function Certificate({ isOpen, onClose, userName, level, date }: CertificateProps) {
  ...
  const getLevelStyles = (lvl: string) => {
    switch (lvl?.toLowerCase()) {
      case "diamante": return { bg: "from-cyan-900 to-blue-900", border: "border-cyan-400", text: "text-cyan-100", accent: "text-cyan-400" }
      case "safira": return { bg: "from-blue-900 to-indigo-900", border: "border-blue-400", text: "text-blue-100", accent: "text-blue-400" }
      default: return { bg: "from-slate-800 to-gray-900", border: "border-slate-400", text: "text-slate-100", accent: "text-slate-300" }
    }
  }
  ...
  return (
    <AnimatePresence>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[70] flex items-center justify-center bg-black/90 backdrop-blur-md p-4">
        <motion.div initial={{ scale: 0.8, opacity: 0, rotateX: 20 }} animate={{ scale: 1, opacity: 1, rotateX: 0 }} transition={{ type: "spring", duration: 0.8 }} className="relative max-w-4xl w-full">
          <button onClick={onClose} className="absolute -top-12 right-0 text-white/50 hover:text-white transition-colors"><X className="h-8 w-8" /></button>
          <div className={`relative bg-gradient-to-br ${styles.bg} p-2 rounded-xl shadow-2xl overflow-hidden border-4 ${styles.border}`}>
            <div className={`border-2 border-dashed ${styles.border} border-opacity-30 rounded-lg p-8 md:p-12 text-center relative`}>
              ...
              <div className="absolute top-0 left-1/2 -translate-x-1/2 w-32 h-32 bg-white/5 blur-3xl rounded-full" />
              ...
              <p className="text-lg text-slate-300 max-w-2xl mx-auto mb-8 leading-relaxed">
                Por demonstrar excelência em networking e atingir o prestigiado nível <span className={`font-bold ${styles.accent} uppercase`}>{level}</span> na plataforma Conecta Empresários.
              </p>
              <div className="flex justify-between items-end mt-12 pt-8 border-t border-white/10">
                ...
                <div className="text-right">
                  <div className="h-12 w-32 relative mb-2 opacity-80">
                    <svg viewBox="0 0 200 60" className="w-full h-full">
                      <path d="M10,50 Q40,10 70,40 T150,30" stroke="white" strokeWidth="2" fill="none" strokeLinecap="round" />
                    </svg>
                  </div>
                  <p className="text-white/60 text-xs uppercase tracking-widest border-t border-white/20 pt-1 inline-block">CEO, Conecta Empresários</p>
                </div>
              </div>
            </div>
          </div>
          <div className="flex justify-center gap-4 mt-8">
            <Button onClick={() => alert("Download iniciado...")} className="bg-white text-slate-900 hover:bg-slate-200"><Download className="mr-2 h-4 w-4" />Baixar PDF</Button>
            <Button onClick={() => alert("Link copiado!")} variant="outline" className="text-white border-white/20 hover:bg-white/10"><Share2 className="mr-2 h-4 w-4" />Compartilhar</Button>
          </div>
        </motion.div>
      </motion.div>
    </AnimatePresence>
