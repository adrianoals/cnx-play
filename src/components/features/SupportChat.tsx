...
const getBotResponse = (input: string): string => {
  ...
  if (lower.includes("plano") || lower.includes("preço") || lower.includes("valor") || lower.includes("assinatura") || lower.includes("quanto custa"))
  ...
  if (lower.includes("suporte") || lower.includes("humano") || lower.includes("ajuda") || lower.includes("atendente"))
    return "Nosso suporte atende pelo WhatsApp: (11) 95022-2063."
  if (lower.includes("cancelar") || lower.includes("cancelamento") || lower.includes("encerrar"))
...
export default function SupportChat() {
  ...
  const [messages, setMessages] = useState([{ id: 1, text: "Olá! 👋 Sou a Sofia, a assistente virtual do Conecta. Posso te ajudar com dúvidas sobre a plataforma?", sender: "bot" }])
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[60] flex flex-col items-end pointer-events-none">
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-950/50 custom-scrollbar">
              ...
              {isTyping && (
                <div className="flex justify-start">
                  <div className="bg-slate-800 border border-slate-700 rounded-2xl rounded-tl-none p-4 shadow-sm flex items-center gap-1.5">
            ...
            <form onSubmit={handleSend} className="p-3 bg-slate-900 border-t border-slate-700 flex gap-2">
              <input type="text" value={inputText} onChange={e => setInputText(e.target.value)} placeholder="Digite sua dúvida..."
                className="flex-1 bg-slate-950 border border-slate-700 rounded-xl px-4 py-2 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/50" />
              <Button type="submit" size="icon" disabled={!inputText.trim() || isTyping} className="bg-blue-600 hover:bg-blue-700 text-white rounded-xl h-10 w-10 shrink-0 shadow-lg shadow-blue-900/20">
      ...
      <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setIsOpen(!isOpen)}
        className={`pointer-events-auto shadow-xl shadow-blue-900/40 flex items-center justify-center rounded-full w-14 h-14 transition-all duration-300 border-2 border-white/10 ${isOpen ? "bg-slate-800 text-white rotate-90 hover:bg-slate-700" : "bg-gradient-to-r from-blue-600 to-purple-600 text-white hover:from-blue-500 hover:to-purple-500"}`}
      >
