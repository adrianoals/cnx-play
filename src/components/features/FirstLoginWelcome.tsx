    ...
    <Dialog open={isOpen} onOpenChange={open => !open && handleClose()}>
      <DialogContent className="max-w-md bg-slate-950 border-slate-800 text-center p-0 overflow-hidden sm:rounded-3xl focus:outline-none">
        <div className="relative p-8">
          ...
          <div className="absolute -top-10 left-1/2 -translate-x-1/2 w-40 h-40 bg-blue-500/20 blur-[50px] rounded-full pointer-events-none" />
          <motion.div initial={{ scale: 0, rotate: -180 }} animate={{ scale: 1, rotate: 0 }} transition={{ type: "spring", duration: 1 }}
            className="w-20 h-20 bg-gradient-to-br from-blue-600 to-purple-600 rounded-2xl mx-auto flex items-center justify-center mb-6 shadow-xl shadow-blue-900/20 relative z-10">
            <Rocket className="h-10 w-10 text-white" />
          </motion.div>
          <AnimatePresence>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
              <h2 className="text-2xl font-bold text-white mb-2">Bem-vindo, {userName}!</h2>
              <p className="text-slate-400 mb-8 leading-relaxed">Estamos felizes em ter você aqui. Preparamos um tour rápido para você conhecer as principais ferramentas da plataforma.</p>
            </motion.div>
          </AnimatePresence>
